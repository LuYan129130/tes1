...
uint8_t PCDNFCT4_ReadNDEF( void )
{
	...
	while (size>MLe)
	{
		errchk(PCDNFCT4_ReadBinary(i*MLe, MLe, buffer));
		memcpy(&CardNDEFfile[i*MLe],&buffer[PCD_DATA_OFFSET+1],MLe);
		size -= MLe;
		i++;
	}
	if (size > 0)
	{
		errchk(PCDNFCT4_ReadBinary(i*MLe, size, buffer));
		memcpy(&CardNDEFfile[i*MLe],&buffer[PCD_DATA_OFFSET+1],size);
	}
