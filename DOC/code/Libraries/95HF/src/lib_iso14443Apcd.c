...
int8_t ISO14443A_IsPresent( void )
{
	...
	/* WakeUp attempt */
	if(ISO14443A_REQA(pDataRead) != ISO14443A_SUCCESSCODE )
	{
		/* try again in case tag was in ready or idle state */
...
int8_t ISO14443A_Anticollision( void )
{
	...
	if(ISO14443A_Card.SAK & SAK_FLAG_ATS_SUPPORTED)
	{
		...
		errchk(ISO14443A_PPS(pDataRead) );

	}
	st95mode = PCD;
	/* Check the Tag type found */
	if (ISO14443A_Card.SAK == 0x00) /* TT2 */
		st95tagtype = TT2;
	else 
		st95tagtype = TT4A;
