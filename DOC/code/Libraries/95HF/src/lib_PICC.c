...
/**
 *	@brief  this function sends an IDN command to the PICC device device
 *  @param  pResponse : pointer on the PICC device reply
 *  @retval PICC_SUCCESSCODE : the function is succesful 
 */
int8_t PICC_POLL_FIELD(uint8_t *pResponse)
{
...
int8_t PICC_SetBackscatteringLoad (uc8 bLoad)
{
	...
	PICC_WriteRegister (0x04,0x68,0x01,pData,u95HFBuffer);
...
int8_t PICC_GetBackscatteringLoad ( void )
{
	...
	PICC_WriteRegister (0x03,0x68,0x00,pData,u95HFBuffer);
	/* read the register*/
	PICC_ReadRegister(0x03,0x69,0x01,0x00,u95HFBuffer);
